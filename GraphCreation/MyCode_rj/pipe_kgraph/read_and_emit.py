...
class StateFileReader:
    ...
    def read_and_emit(self):
        ...
        for file_path in self.state_files:
            ...
            file_basename = os.path.basename(file_path)  # Get the filename
            state = {}
            current_t = None
                ...
                for _, row in batch_df.iterrows():
                    if current_t is None:
                        ...
                        if state:  # Ensure state is not empty before putting
                            package = {'data': deepcopy(state), 'source_file': file_basename}
            ...
            if state:
                package = {'data': state, 'source_file': file_basename}
                self.buffer.put(package)
